...
Install dependencies — one-time package installs.

Setup — initializes the embedding model, Supabase client, and Gemini client.

Scraping + chunking functions — scrape_page(), chunk_blocks().

Ingest pipeline — ingest_pages(), with the duplicate-content guard.

Retrieval — retrieve_chunks(), calls the match_documents Postgres RPC.

Generation — build_context() and ask_rag(), which ties retrieval and generation together.

Usage examples — commented-out calls, run manually and one at a time to control API usage.

Cells 1–6 only need to be re-run after a runtime restart or disconnect; they define functions and clients but don't scrape, embed, or call the LLM on their own. Only Cell 7's uncommented calls cost time or API quota.

Example Queries
python
answer, sources = ask_rag("How do I connect Spring Boot to a MongoDB database?")
answer, sources = ask_rag("What does Spring Boot auto-configure by default when Spring Security is on the classpath?")
answer, sources = ask_rag("How does Spring Boot handle servlet filters?")
